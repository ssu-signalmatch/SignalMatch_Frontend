...
val sampleUiState = GetBookmarkUiState(
    isLoading = false,
    bookmark = listOf(
        GetBookmarkData(targetType = "", targetId = 1, name = "샘플 기업 A"),
        GetBookmarkData(targetType = "", targetId = 2, name = "샘플 기업 B")
    )
)
            ...
            uiState.bookmark.isEmpty() -> {
                ...
                LazyColumn(
                    ...
                    items(uiState.bookmark) { item ->
                        MypageCard3(
                            text = item.name,
                            onClick = { deleteId = item.targetId }
                        )
...
fun BookmarkPreview(){
    ...
    BookmarkScreen( navController, sampleUiState, onRefresh = { }, onConfirmDelete = { })
}